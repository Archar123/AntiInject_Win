...
typedef struct _ANTI_INJECT_BLACK_WHITE_DATA
{
    ULONG m_ProcessId;
    LIST_ENTRY m_Entry;
    WCHAR m_ProcessName[MAX_PATH];
}ANTI_INJECT_BLACK_WHITE_DATA, * PANTI_INJECT_BLACK_WHITE_DATA;

typedef struct _ANTI_INJECT_PROTECT_PROCESS_DATA
{
    LIST_ENTRY m_Entry;
    PEPROCESS m_Eprocess;
    WCHAR m_Name[MAX_PATH];

}ANTI_INJECT_PROTECT_PROCESS_DATA, *PANTI_INJECT_PROTECT_PROCESS_DATA;
